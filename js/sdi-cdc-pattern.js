...
4. **Cross-account trust via Workload Identity Federation (no stored keys).** The AWS ↔ warehouse handshake is a **WIF** pattern: instead of minting an IAM user's long-lived access key and pasting it into the warehouse, the warehouse's **storage integration** presents its own cloud-issued identity, and an AWS IAM role is configured to **let that identity assume it** — keyed on an **external ID**. Snowflake's storage integration surfaces an AWS IAM user ARN + external ID; the S3 bucket's role trust policy allows \`sts:AssumeRole\` only from *that* principal *with that* external ID, and hands back short-lived STS credentials for each load. Nothing long-lived is stored on either side.
   - **Why the external ID matters** — it's the standard **confused-deputy** defense: it ties the assumed role to *this specific* integration, so another tenant of the same multi-tenant service can't trick the warehouse into assuming your role. (This is exactly the "pin the claims, don't just trust the issuer" rule from the WIF fundamentals.)
   - **The alternative you're rejecting** — an IAM access key in the warehouse's secret store. It's copyable, rarely rotated, and a single leak grants your S3 data forever. Federation removes the secret entirely; the only credential in flight is a minutes-long STS token bound to the integration. *(See the CS Fundamentals guide "Workload Identity Federation (WIF)" for the general token-exchange model.)*
...
"Transactional data lives in DynamoDB; analysts need warehouse-scale queries without touching production. So we CDC it: DynamoDB Streams → EventBridge Pipes (one per table, own retry) → a shared enrichment Lambda that unmarshals and stamps audit metadata → Firehose batches and time-partitions to S3 → Snowpipe auto-ingests into a single landing table with a raw \`VARIANT\` payload and a \`_TYPE\` discriminator. Per-entity typed views project and dedup with \`QUALIFY ROW_NUMBER()\`. The whole thing is feature-flagged per environment, cross-account trust is **workload identity federation** — the warehouse's storage integration assumes an IAM role keyed on an external ID, so there are no stored keys — and \`prevent_destroy\` guards the stage/table/pipe because losing \`LOAD_HISTORY\` means duplicate ingestion."
`;
