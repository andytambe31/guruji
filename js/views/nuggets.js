...
const NUGGETS = [
  ...
  // ---------- System Design: foundations (the "type a URL" path + scaling) ----------
  { id: 'sd-request-path', area: 'System Design', topics: ['url', 'request', 'http', 'system design', 'network'], front: 'What typing a URL actually triggers', body: 'Parse the URL → DNS resolves the host to an IP → TCP handshake → TLS handshake → HTTP request → a load balancer routes to an app server → it hits a cache, then a database → the response comes back → the browser renders it. Every system-design concept hangs off one of those hops.' },
  { id: 'sd-dns', area: 'System Design', topics: ['dns', 'url', 'system design', 'network'], front: 'How DNS finds the server', body: 'A hostname becomes an IP before any request leaves: browser cache → OS cache → resolver → root → TLD → authoritative nameserver, each answer cached by its TTL. It’s a paid 3rd-party service, and a slow or stale lookup is a classic hidden source of latency and outages.' },
  { id: 'sd-dns-8888', area: 'System Design', topics: ['dns', 'resolver', 'system design', 'network'], front: 'What is 8.8.8.8?', body: 'Google’s public recursive DNS resolver. A recursive resolver takes your query and does the legwork for you — it contacts root, TLD, and authoritative servers as needed, caches the results, and returns the final IP. So your machine makes one request instead of walking the chain itself.' },
  { id: 'sd-handshake', area: 'System Design', topics: ['tcp', 'tls', 'handshake', 'system design', 'network'], front: 'TCP then TLS, in that order', body: 'TCP’s 3-way handshake (SYN / SYN-ACK / ACK) opens a reliable, ordered pipe; TLS then makes it private — verify the cert, agree a fast symmetric session key (one round trip in TLS 1.3). Reusing the connection (keep-alive) skips both next time — the cheapest latency win there is.' },
  { id: 'sd-tls-handshake', area: 'System Design', topics: ['tls', 'https', 'handshake', 'ecdhe', 'certificate', 'system design', 'network', 'security'], front: 'The TLS handshake, step by step', body: 'After TCP connects: ClientHello offers TLS versions + cipher suites and the SNI (so one IP can host many HTTPS sites). ServerHello returns the certificate chain + ECDHE params; the browser verifies hostname, trust chain to a root CA, expiry, and revocation. Via ECDHE both sides derive the *same* symmetric session key without ever sending it — and get forward secrecy (past sessions stay safe even if the server’s private key later leaks). All app data then rides fast symmetric encryption. TLS 1.3 needs one round trip after TCP; 0-RTT resumption saves another for returning clients, at the cost of that early data being replayable.' },
  { id: 'sd-replication', area: 'System Design', topics: ['replication', 'master', 'replica', 'database', 'system design'], front: 'Master takes writes, replicas take reads', body: 'Replicate the same data to read replicas → reads scale out and you gain redundancy (most apps read far more than they write). If the master dies, a replica is promoted — but its data may be slightly stale, so promotion needs recovery, not just a flip.' },
  { id: 'sd-vert-horiz', area: 'System Design', topics: ['scaling', 'vertical', 'horizontal', 'system design', 'scale'], front: 'Scale up vs scale out', body: 'Vertical (scale up) = a bigger box — simple, but a hard ceiling and a single point of failure. Horizontal (scale out) = more boxes behind a load balancer — near-limitless and failover-friendly, but it forces you to keep the tier stateless. Real systems scale out.' },
  { id: 'sd-stateless', area: 'System Design', topics: ['stateless', 'session', 'scaling', 'system design', 'autoscale'], front: 'Keep the web tier stateless', body: 'If a server remembers your session locally, every request must pin to that server (sticky sessions) — painful to scale and fragile on failure. Push session state to a shared store (Redis/DB) so any server can serve any request. That’s what makes autoscaling and failover trivial.' },
  { id: 'sd-redundancy', area: 'System Design', topics: ['redundancy', 'spof', 'availability', 'system design', 'failover'], front: 'No single point of failure', body: 'One of anything — one DB, one cache, one load balancer — is a SPOF: it fails, the system fails. Put at least two at every tier, ideally across zones/data centers, so losing one degrades you instead of downing you. Availability is redundancy plus a way to fail over.' },
  { id: 'sd-cdn-edge', area: 'System Design', topics: ['cdn', 'edge', 'static', 'cache', 'system design'], front: 'A CDN serves static bytes from the edge', body: 'Cache images/CSS/JS/video on servers physically near the user. First request fills the edge from origin (with a TTL); everyone after is served locally — huge latency win, less origin load. To push an update before the TTL, version the URL (image.png?v=2) or call the invalidation API.' },

  // ---------- System Design: the "type a URL" follow-ups ----------
  { id: 'sd-fu-cache-where', area: 'System Design', topics: ['cache', 'invalidation', 'ttl', 'system design'], front: 'Where do you cache — and how do you bust it?', body: 'Cache at every layer you can tolerate staleness: browser → CDN edge → an app store (Redis) → the DB’s own cache. Invalidation is the hard half — two strategies: TTL (expire after N seconds; simple, briefly stale) or explicit purge / write-through (always fresh, but more coupling and moving parts). Decide the staleness budget per layer — “cache invalidation” is famously one of the two hard problems in CS.' },
  { id: 'sd-fu-http-versions', area: 'System Design', topics: ['http', 'http2', 'http3', 'quic', 'system design', 'network'], front: 'HTTP/1.1 vs 2 vs 3', body: '1.1: effectively one request at a time per connection → head-of-line blocking (browsers open many connections to fake parallelism). 2: multiplexes many streams over one TCP connection — but a single lost TCP packet still stalls every stream (HOL blocking just moved down to TCP). 3: runs over QUIC (UDP), so streams are independent — no transport-level HOL blocking, plus a faster combined transport+TLS handshake.' },
  { id: 'sd-fu-100x', area: 'System Design', topics: ['scale', 'bottleneck', 'database', 'system design'], front: 'What breaks first at 100× traffic?', body: 'Almost always the stateful tier — the database, or one hot service/shard everything funnels through. Stateless web servers scale out behind a load balancer trivially; the DB doesn’t. Expect to add read replicas, put a cache in front, then shard — and watch for one hot key (the celebrity problem) melting a single shard.' },
  { id: 'sd-fu-second-visit', area: 'System Design', topics: ['cache', 'latency', 'dns', 'cdn', 'system design', 'network'], front: 'First visit vs second visit', body: 'The second is far faster because caches are warm: DNS is cached (no resolver walk), the TCP/TLS connection may be reused — or TLS resumes, even 0-RTT — CDN edges already hold the static assets, and conditional requests return 304 Not Modified instead of re-downloading. The cold first visit pays for all of it; returning visitors ride the warm path.' },
  { id: 'sd-fu-https-protects', area: 'System Design', topics: ['https', 'tls', 'security', 'system design', 'network'], front: 'What does HTTPS actually protect?', body: 'Confidentiality + integrity of data *in transit* — an eavesdropper can’t read or tamper with it, and you’ve authenticated the server’s identity. That’s it. It does NOT protect data at rest on the server, a compromised endpoint, what the server does with your data, or some metadata (SNI/DNS can leak which host you contacted). HTTPS secures the pipe, not the ends.' },
